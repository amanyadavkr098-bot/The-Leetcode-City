export type CitySerializableValue =
  | string
  | number
  | boolean
  | null
  ...
  | { [key: string]: CitySerializableValue };

export type CityDeveloperLike = Record<string, CitySerializableValue> & {
...
function isObject(value: CitySerializableValue): value is Record<string, CitySerializableValue> {
  return typeof value === "object" && value !== null && !Array.isArray(value);
}

function isLoadout(value: CitySerializableValue): value is { crown?: string | null; roof?: string | null; aura?: string | null; faces?: string | null } {
  return isObject(value);
}

function isRaidTag(value: CitySerializableValue): value is { attacker_login?: string; tag_style?: string; expires_at?: string } {
  return isObject(value);
}

export class CitySerializer {
  serializeDeveloper(dev: CityDeveloperLike): Record<string, CitySerializableValue> {
    const result: Record<string, CitySerializableValue> = {};
